...
복잡한 경제 뉴스를 명확하게 정리하는 경제 미디어 사이트입니다.
Next.js(App Router) + TypeScript + Tailwind CSS + MDX로 만들어졌고, 데이터베이스나 CMS 없이
`content/` 폴더의 `.mdx` 파일만으로 운영되는 **정적 생성(SSG) 기반 미디어 시스템**입니다.
...
│   ├── today/                  # 오늘의 경제 (일간 브리핑)
│   ├── learn/                  # 경제 공부 (topic 필드로 서브카테고리 필터링)
│   ├── invest/                 # 투자 분석
│   ├── terms/                  # 경제 용어 (사전형)
│   ├── blog/                   # 블로그 (자유 주제)
│   └── README.md               # 글쓰기 규칙 문서
...
│   ├── app/                    # 라우트 (App Router)
│   │   ├── page.tsx             # 홈
│   │   ├── {category}/page.tsx        # 카테고리 허브(목록) 페이지 × 5
│   │   ├── {category}/[slug]/page.tsx # 글 상세 페이지 × 5
│   │   ├── about/page.tsx       # 소개
...
│   │   ├── ui/                  # PostCard, Badge, Thumbnail 등 UI 단위 컴포넌트
│   │   ├── PostDetail.tsx        # 글 상세 페이지 본문 조립 (5개 상세 라우트가 공유)
...
│       ├── site.ts               # 사이트 정보, 카테고리 라벨/컬러 테마, 내비게이션
...
카테고리는 5개로 고정되어 있고, 각각 고유한 브랜드 컬러를 가집니다(`src/lib/site.ts`의 `CATEGORY_THEME`).

| 카테고리 | 경로     | 컬러   | 성격                          |
| -------- | -------- | ------ | ----------------------------- |
| today    | `/today` | blue   | 일간 시황 브리핑                |
| learn    | `/learn` | purple | 경제 공부 (topic 필터: 주식/부동산/금리·채권/세금/지표/국제경제) |
| invest   | `/invest`| green  | 투자 분석                      |
| terms    | `/terms` | gray   | 경제 용어 사전                  |
| blog     | `/blog`  | orange | 자유 주제                      |
...
- **OpenGraph / Twitter 메타**: 모든 페이지에 `title`/`description`/`openGraph`/`twitter` 완비.
  글 상세 페이지의 OG 이미지는 `src/app/og/route.tsx`가 카테고리 색 + 제목을 넣어 요청마다
...
1. `content/{today|learn|invest|terms|blog}/` 중 하나에 `새-글-슬러그.mdx` 파일을 만듭니다.
2. frontmatter에 `title`/`date`/`description`/`tags`를 채웁니다(`learn`은 `topic`도 필수).
...
- `learn` 카테고리는 `topic` 필수 (`LEARN_TOPICS` 슬러그와 정확히 일치)
- `summary`(2~4개 배열)를 넣으면 상세 페이지 상단에 "한눈에 보기" 요약 박스가 자동 생성됩니다
- 본문은 순수 마크다운(`##`, 목록, 표, 굵게 등)이면 충분합니다. 자세한 스키마 표와 템플릿은
...
3. 이미지를 넣지 않아도 카테고리 색 플레이스홀더가 자동으로 표시되므로, 이미지 없이 먼저 발행하고
   나중에 추가해도 됩니다.
...
새 카테고리(예: `chart` — 차트 분석)를 추가하려면, 콘텐츠만으로는 안 되고 **코드 수정이 필요**합니다.
아래 순서를 그대로 따르세요.

1. **콘텐츠 폴더 생성**: `content/chart/`를 만들고 샘플 `.mdx` 파일을 1개 넣습니다.
2. **타입 등록**: `src/lib/types.ts`의 `CATEGORIES` 배열에 `"chart"`를 추가합니다.
   ```ts
   export const CATEGORIES = ["today", "learn", "invest", "terms", "blog", "chart"] as const;
   ```
3. **라벨/컬러/내비게이션 등록**: `src/lib/site.ts`에서
   - `CATEGORY_LABELS`에 `chart: "차트 분석"` 추가
   - `CATEGORY_THEME`에 원하는 색상 팔레트로 8개 필드(`soft`/`softBorder`/`text`/`textStrong`/
     `textHover`/`groupHoverText`/`border`/`borderHover`/`accentBorderL`)를 채워 추가
     (기존 5개 카테고리 항목을 복사해서 색상 이름만 바꾸는 것이 가장 안전합니다 — Tailwind가
     클래스명을 정적으로 스캔하므로 각 값은 완전한 클래스 문자열이어야 합니다)
   - `NAV_ITEMS`에 `{ label: "차트 분석", href: "/chart" }` 추가
4. **라우트 생성**: `src/app/chart/page.tsx`와 `src/app/chart/[slug]/page.tsx`를 만듭니다.
   기존 `src/app/invest/page.tsx`, `src/app/invest/[slug]/page.tsx`를 복사한 뒤
   `category="invest"` → `category="chart"`, 관련 문자열만 바꾸면 됩니다.
5. **빌드 확인**: `npm run build`로 `/chart`, `/chart/{슬러그}`가 정상 생성되는지 확인합니다.
...
- **꾸준히**: `terms`/`learn`처럼 시간이 지나면 내용이 달라지는 글은 내용을 고칠 때 frontmatter
